...
export default function TVBoardPage() {
  ...
  const latestSale = boardData?.recentSales && boardData.recentSales.length > 0 
    ? boardData.recentSales[boardData.recentSales.length - 1] 
    : null;
  const bellRingerName = latestSale?.name || "No Sales Yet";
  ...
  return (
    <div className={`w-screen h-screen ${getThemeWrapperClass()} flex items-center justify-center overflow-hidden transition-colors duration-1000`}>
      ...
      <CelebrationOverlay 
        ...
        agentAvatarUrl={celebratingAgentAvatarUrl}
        soundUrl={boardData?.company?.celebrationSoundUrl || null} 
        soundStartTime={boardData?.company?.celebrationSoundStartTime || 0}
        soundDuration={boardData?.company?.celebrationSoundDuration || 10}
      />
