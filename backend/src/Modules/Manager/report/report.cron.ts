...
export function initReportCron(prisma: PrismaClient, reportService: ReportService) {
  // Run every 5 minutes to accurately catch the 10-minutes-before-closing window
  cron.schedule("*/5 * * * *", async () => {
    ...
    try {
      ...
      for (const manager of managers) {
        ...
        // The cron job should ALWAYS run every day at 11:45 PM, regardless of whether the manager clicked "Start New Day"
        try {
          ...
          // We want to generate the report 10 minutes before the office closes in THEIR timezone.
          const closeTimeStr = manager.company.settings.officeCloseTime || "17:00";
          const [closeH, closeM] = closeTimeStr.split(":").map(Number);
          
          let targetMinute = closeM - 10;
          let targetHour = closeH;
          if (targetMinute < 0) {
            targetMinute += 60;
            targetHour -= 1;
          }
          ...
          const currentTotalMinutes = currentHourTz * 60 + currentMinuteTz;
          const targetTotalMinutes = targetHour * 60 + targetMinute;

          // Since the cron runs every 5 minutes, we check if current time falls within a 5-min window of target time
          if (currentTotalMinutes >= targetTotalMinutes && currentTotalMinutes < targetTotalMinutes + 5) {
            ...
            const alreadyGenerated = await prisma.reportHistory.findFirst({
              where: {
            ...
            // Optionally reset isReportGenerationActive to true for the next day
            if (!isReportActive) {
              await prisma.companySettings.update({
                where: { companyId: manager.company.id },
                data: { isReportGenerationActive: true }
              });
            }
          }
        } catch (e: any) {
