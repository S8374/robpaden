...
// import { useUpdateAgentMutation, useDeleteAgentMutation, useUploadFileMutation } from "@/redux/api/agent.api";
...
export function EditAgentModal({ isOpen, onClose, agent }: EditAgentModalProps) {
  ...
  useEffect(() => {
    if (agent) {
      ...
      setFormData({ 
        name: agent.name || "",
        role: "Sales Representative", // Mock role
        dailyGoal: agent.today ? String(agent.today) : "", // Just mapping mock fields
        weeklyGoal: agent.week ? String(agent.week) : "",
        initials: agent.initials || "AB",
      });
    }
  }, [agent]);
        ...
        <div className="px-6 pb-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="text-[#5252ff] hover:text-[#4242e5] font-semibold text-sm transition-colors">
                Upload Photo
              </button>
            </div>
          </div>
