...
export function SalesCard({ title, value, subtitle, bars, isLoading }: SalesCardProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="h-48 bg-white rounded-2xl border border-zinc-200 shadow-sm hover:shadow-md transition-shadow duration-300 p-5 flex flex-col justify-between group">
      <div>
      ...
      <div className="flex flex-col mt-auto gap-2">
        <div className="flex items-end gap-3 h-12 w-full">
          {bars.map((bar, i) => (
            <div 
              key={i} 
              className={`flex-1 rounded-full transition-colors ${bar.active ? 'bg-[#5252ff]' : 'bg-[#eef0ff]'}`}
